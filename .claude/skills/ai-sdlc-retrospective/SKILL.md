...
- D: use [the owning Python entry point](scripts/retrospective.py) with explicit inputs; success covers only executed checks.
- S: interpret sources for `retrospective.md` and `_ai_sdlc/retrospective.toon`; cite unresolved decisions.
...
Primary: Observation / Evidence / Improvement proposal / Decision status / Owner; secondary: Proposal / Decision reference / Next action / Owner.
Rows represent individual observation records. Show the user decision before detail; preserve source order and explicit authority.
Summary: Status / Decision / Evidence. Failure: Observation evidence / Status / Blocker / Evidence / Required action.
Clarification: Missing observation evidence / Why required / Known evidence / Options. Next action: Owner / Next action / Expected evidence.
