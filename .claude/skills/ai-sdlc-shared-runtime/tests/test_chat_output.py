...
class ChatOutputTests(unittest.TestCase):
    ...
    def test_failure_clarification_and_summary_mutations(self):
        suite=chat.read(self.root/'tests/fixtures'/('chat-scenarios.json' if (self.root/'tests/fixtures/chat-scenarios.json').exists() else 'chat-scenarios.toon'))
    ...
    def test_workflow_presentation_retains_handoffs_and_stops_on_blockers(self):
        ...
        for name in names:
            ...
            suite=chat.read(fixture if fixture.exists() else fixture.with_suffix('.json'))
