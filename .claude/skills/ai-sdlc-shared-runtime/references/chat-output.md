...
Read only `references/chat-output.json` for the selected skill. Render verified
facts with the sibling `scripts/chat_output.py render --skill-root <skill>
--input <chat-result.json>`; check a draft with `check --skill-root <skill>
...
use host question tools where available. Native commands, patches, TOON, JSON,
YAML, code and commit messages keep their original syntax in fenced blocks or
